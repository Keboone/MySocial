...
@WebServlet("/sendMessageServlet2")
public class sendMessageServlet2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try
		{
			...
			if(message != null)
			{
				stmt.executeUpdate("INSERT INTO message VALUES (NULL, '"+from+"', '"+to+"', '"+message+"', "+2+")");
				
				request.getRequestDispatcher("secondConversation.jsp").include(request, response);
			}
			else
			{
				request.getRequestDispatcher("conversationError.jsp").include(request, response);
			}
			out.close();
		}
		catch (Exception e) 
